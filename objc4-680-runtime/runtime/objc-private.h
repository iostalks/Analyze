...
#   define ISA_MAGIC_VALUE 0x000001a000000001ULL
    struct {
        uintptr_t nonpointer        : 1; //~ 是否开启 isa 指针优化，优化后为 Tagged Pointer
        uintptr_t has_assoc         : 1; //~ 是否含有关联对象，没有关联对象可以更快的释放内存
        uintptr_t has_cxx_dtor      : 1; //~ 是否有 C++/Objc 析构器
        uintptr_t shiftcls          : 33; // MACH_VM_MAX_ADDRESS 0x1000000000 //~ 虚拟内存最大地址，代表类指针
        uintptr_t magic             : 6; //~ 是否初始化完成，arm64 中 0x16 代表对象已经初始化
        uintptr_t weakly_referenced : 1; //~ 是否弱引用其他对象 or 被别人弱引用？
        uintptr_t deallocating      : 1; //~ 是否正在释放
        uintptr_t has_sidetable_rc  : 1; //~ 引用计数是否过大，如果过大需要其他散列表来存储
        uintptr_t extra_rc          : 19; //~ 引用计数减 1 的值，如果引用计数为 5，该值即为 4
